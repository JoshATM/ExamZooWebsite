...
export default function SignUp() {
  ...
  const Submit = async (e) => {
    ...
    if ((firstname && lastname && email && password && confPassword) === "") {
      ...
      toast.error("Passwords do not match, please make sure you enter them correctly!");
    } else if (password.length < 8) {
      toast("Please make your password longer than 8 characters!");
    } else {
      try {
        // Email validation to check if it already exists
        const emailAlreadyExists = await fetch(`http://localhost:5000/check-email/${email}`, {
          method: "GET",
          headers: {
  ...
  return (
    <>
    <StyledContainer>
      <StyledImage src={WomanWithGoats} alt='Woman taking picture with Goats'/>
